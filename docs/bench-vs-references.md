...
a useful smoke-validation signal. Full biological impact still depends on
topic modelling, motif enrichment and eRegulon checks.
...
- `topics.fit_gibbs`: Collapsed Gibbs LDA (Griffiths-Steyvers 2004),
  closer to the Mallet algorithm class at K >= 30
...
gensim's pure-numpy VB wins on raw wall-clock at this shape. RustScenic
provides both topic paths, so the choice is speed or topic diversity.
...
**Collapsed Gibbs gives 11× more distinct topics than Online VB in this
...
It is closer to Mallet's algorithm class than Online VB and does not require
Java.
...
- **Two algorithms**: VB for speed at K <= 10, Gibbs for topic diversity at
  K >= 30.
...
| GRN end-to-end | arboreto | rustscenic.grn.infer | **faster in this run, but fine edge ranks differ** |
| Memory at 100k cells × 20k genes × 4 stages | scenicplus stack > 40 GB (reported) | 6.3 GB | **reported context, not same-hardware proof** |
