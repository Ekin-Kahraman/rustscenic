...
scATAC in our K>=30 benchmarks, where only a small fraction of requested
topics carry any cell. v0.3
...
This is not a rustscenic-specific bug. The same pattern appears in gensim's
`LdaModel` on similar data in our benchmarks. SCENIC+ methods report
pycisTopic default parameters using MALLET for topic modelling, so
`fit_gibbs` is the closer algorithmic match for fine-grained K>=30 runs.
...
Based on these measurements and the known behaviour of variational LDA on
sparse corpora, parameter tuning is not expected to lift unique-topic count
into Mallet's range. The practical fix is a different sampler, not a larger
grid search.
...
Both return the same `TopicsResult` shape. Choose based on
your priority.
...
- v0.4.x: parallel AD-LDA Gibbs (`fit_gibbs(..., n_threads=N)`),
