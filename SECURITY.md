...
If you find a vulnerability, such as memory unsafety, arbitrary file write, crash on crafted input, or supply-chain concern, please **do not** open a public issue.
...
- Rust dependencies are pinned in `Cargo.lock`. CI runs `cargo audit`.
...
- Wheels are built via GitHub Actions with trusted-publisher OIDC to PyPI. No API tokens are stored in the repo.
