#!/usr/bin/env bash
# Fresh real-data audit for rustscenic v0.4.3 — 10x PBMC granulocyte-sorted
# 10k human multiome. Third real dataset on record, different cell composition
# than the v0.3.9 PBMC 3k unsorted run.
#
# Test posture: install the v0.4.3 wheel from the local branch (not PyPI; the
# release/v0.4.3-cleanup branch hasn't tagged yet), run the public
# pipeline.run, hard-assert every stage non-empty, check canonical
# PBMC-with-granulocytes TFs (myeloid + lymphoid), serialise an evidence-
# schema-compliant artefact JSON.
#
# Dataset: 10x cellranger-arc 2.0.0 pbmc_granulocyte_sorted_10k
#   https://www.10xgenomics.com/datasets/pbmc-from-a-healthy-donor-granulocytes-removed-through-cell-sorting-10-k-1-standard-2-0-0

set -euo pipefail

DATA_DIR="$HOME/projects/bio/rustscenic/validation/real_multiome_pbmc_granulocyte_10k"
ARTEFACT="$HOME/projects/bio/rustscenic/validation/multiome_pipeline_run_v0.4.3_pbmc_granulocyte_10k.json"
REPO="$HOME/projects/bio/rustscenic"
WORK=$(mktemp -d)
echo "work dir: $WORK"

python3 -m venv "$WORK/venv"
source "$WORK/venv/bin/activate"
pip install --quiet --upgrade pip
# Use the local editable build at the current branch HEAD (we are auditing
# release/v0.4.3-cleanup pre-tag); pin extras to match the released-line
# pattern.
pip install --quiet "${REPO}[validation]"

RUSTSCENIC_SHA=$(cd "$REPO" && git rev-parse HEAD)
RUSTSCENIC_BRANCH=$(cd "$REPO" && git rev-parse --abbrev-ref HEAD)
PYVER=$(python -c "import sys; print('.'.join(map(str, sys.version_info[:3])))")
SCANPY_VER=$(python -c "from importlib.metadata import version; print(version('scanpy'))")
ANNDATA_VER=$(python -c "from importlib.metadata import version; print(version('anndata'))")
RUSTSCENIC_VER=$(python -c "from importlib.metadata import version; print(version('rustscenic'))")
OS=$(uname -srm)
CPU=$(sysctl -n machdep.cpu.brand_string 2>/dev/null || cat /proc/cpuinfo 2>/dev/null | grep -m1 "model name" | sed 's/.*: //' || echo "unknown")
N_CPUS=$(python -c "import os; print(os.cpu_count())")
echo "rustscenic=${RUSTSCENIC_VER} branch=${RUSTSCENIC_BRANCH} sha=${RUSTSCENIC_SHA} cpus=${N_CPUS}"

WORK="$WORK" RUSTSCENIC_SHA="$RUSTSCENIC_SHA" RUSTSCENIC_BRANCH="$RUSTSCENIC_BRANCH" PYVER="$PYVER" \
SCANPY_VER="$SCANPY_VER" ANNDATA_VER="$ANNDATA_VER" RUSTSCENIC_VER="$RUSTSCENIC_VER" \
OS="$OS" CPU="$CPU" N_CPUS="$N_CPUS" \
DATA_DIR="$DATA_DIR" ARTEFACT="$ARTEFACT" \
python - <<'PY'
import json, os, resource, time, hashlib, signal
from pathlib import Path
import anndata as ad
import scanpy as sc
import pandas as pd
import numpy as np
import rustscenic, rustscenic.data, rustscenic.preproc, rustscenic.pipeline

DATA = Path(os.environ["DATA_DIR"])
RNA_H5 = DATA / "pbmc_granulocyte_sorted_10k_filtered_feature_bc_matrix.h5"
ATAC_FRAG = DATA / "pbmc_granulocyte_sorted_10k_atac_fragments.tsv.gz"
PEAKS_BED = DATA / "pbmc_granulocyte_sorted_10k_atac_peaks.bed"
WORK = Path(os.environ.get("WORK", "/tmp")); OUT = WORK / "out"; OUT.mkdir(parents=True, exist_ok=True)

TOTAL_TIMEOUT_S = 90 * 60   # 90 min budget for ~10k cells, human genome

class Timeout(Exception): pass
def _h(s, f): raise Timeout("pipeline.run exceeded TOTAL_TIMEOUT_S")
signal.signal(signal.SIGALRM, _h)

def md5(p):
    h = hashlib.md5()
    with open(p, "rb") as f:
        for chunk in iter(lambda: f.read(8192), b""):
            h.update(chunk)
    return h.hexdigest()

print(f"rustscenic {os.environ['RUSTSCENIC_VER']} (sha {os.environ['RUSTSCENIC_SHA'][:7]}, branch {os.environ['RUSTSCENIC_BRANCH']}) on Python {os.environ['PYVER']}", flush=True)

# --- Pre-pipeline setup ---
t_setup = time.monotonic()
print("[setup 1/3] load + RNA QC", flush=True)
rna = sc.read_10x_h5(RNA_H5); rna.var_names_make_unique()
sc.pp.filter_cells(rna, min_genes=200)
sc.pp.filter_genes(rna, min_cells=3)
rna.var["mt"] = rna.var_names.str.startswith("MT-")
sc.pp.calculate_qc_metrics(rna, qc_vars=["mt"], inplace=True)
rna = rna[rna.obs["pct_counts_mt"] < 20].copy()
sc.pp.normalize_total(rna, target_sum=1e4); sc.pp.log1p(rna)
print(f"  RNA shape post-QC: {rna.shape}", flush=True)

print("[setup 2/3] build ATAC matrix and subset to RNA-QC'd cells", flush=True)
atac_full = rustscenic.preproc.fragments_to_matrix(ATAC_FRAG, PEAKS_BED)
shared = sorted(set(rna.obs_names) & set(atac_full.obs_names))
print(f"  shared barcodes RNA ∩ ATAC: {len(shared):,}", flush=True)
adata_atac = atac_full[shared].copy()
del atac_full
rna = rna[shared].copy()
print(f"  ATAC subset: {adata_atac.shape}", flush=True)

print("[setup 3/3] download human motif rankings + gene coords + TFs", flush=True)
motif_rankings = rustscenic.data.download_motif_rankings(species="human", verbose=False)
gene_coords = rustscenic.data.download_gene_coords(species="hs", verbose=False)
hs_tfs = rustscenic.data.tfs(species="hs")
print(f"  motif_rankings: {motif_rankings.shape}  gene_coords: {gene_coords.shape}  TFs: {len(hs_tfs)}", flush=True)

setup_wall = time.monotonic() - t_setup
print(f"  setup wall: {setup_wall:.1f}s", flush=True)

# --- Single pipeline.run call ---
print("\n[pipeline.run] PBMC granulocyte-sorted 10k — full SCENIC+ E2E", flush=True)
print(f"  timeout: {TOTAL_TIMEOUT_S}s", flush=True)
signal.alarm(TOTAL_TIMEOUT_S)
t0 = time.monotonic()
try:
    result = rustscenic.pipeline.run(
        rna=rna,
        output_dir=OUT,
        adata_atac=adata_atac,
        motif_rankings=motif_rankings,
        gene_coords=gene_coords,
        tfs=hs_tfs,
        grn_n_estimators=100,
        topics_n_topics=15,            # slightly higher than 10 for PBMC-with-granulocytes diversity
        topics_n_passes=3,
        cistarget_top_frac=0.05,
        cistarget_auc_threshold=0.05,
        enhancer_max_distance=500_000,
        enhancer_min_abs_corr=0.1,
        eregulon_min_target_genes=2,
        eregulon_min_enhancer_links=1,
        seed=777,
        verbose=True,
    )
finally:
    signal.alarm(0)
pipeline_wall = time.monotonic() - t0
peak_rss_kb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
peak_rss_gb = peak_rss_kb / (1024**3) if peak_rss_kb > 1e6 else peak_rss_kb / (1024**2)
print(f"\n[pipeline.run done] wall={pipeline_wall:.1f}s peak_rss={peak_rss_gb:.2f}GB", flush=True)

# --- Inspect outputs ---
def file_info(p):
    if p is None: return None
    p = Path(p)
    return {"path": p.name, "exists": p.exists(), "size_bytes": p.stat().st_size if p.exists() else None}

artefact_paths = {
    "atac_matrix_path": result.atac_matrix_path,
    "grn_path": result.grn_path,
    "candidate_regulons_path": getattr(result, "candidate_regulons_path", None),
    "regulons_path": result.regulons_path,
    "pruned_regulons_path": getattr(result, "pruned_regulons_path", None),
    "aucell_path": result.aucell_path,
    "topics_dir": result.topics_dir,
    "cistarget_path": result.cistarget_path,
    "enhancer_links_path": result.enhancer_links_path,
    "eregulons_path": result.eregulons_path,
    "integrated_adata_path": result.integrated_adata_path,
}
output_inventory = {k: file_info(v) for k, v in artefact_paths.items()}
print("\n=== output inventory ===", flush=True)
for k, v in output_inventory.items():
    print(f"  {k}: {v}", flush=True)

# --- Headlines ---
n_grn_edges = int(pd.read_parquet(result.grn_path).shape[0]) if result.grn_path else 0
n_regulons = int(getattr(result, "n_regulons", 0))
n_eregulons = int(getattr(result, "n_eregulons", 0) or 0)
n_candidate_regulons = int(getattr(result, "n_candidate_regulons", 0) or 0)
n_pruned_regulons = getattr(result, "n_pruned_regulons", None)
n_cistarget_rows = int(pd.read_parquet(result.cistarget_path).shape[0]) if result.cistarget_path else 0
n_enhancer_rows = int(pd.read_parquet(result.enhancer_links_path).shape[0]) if result.enhancer_links_path else 0
regulon_source = getattr(result, "regulon_source", "unknown")

print(f"\n=== headline ===", flush=True)
print(f"  GRN edges: {n_grn_edges:,}", flush=True)
print(f"  candidate regulons: {n_candidate_regulons:,}", flush=True)
print(f"  active regulons: {n_regulons:,}  (source={regulon_source})", flush=True)
print(f"  pruned regulons: {n_pruned_regulons}", flush=True)
print(f"  cistarget rows: {n_cistarget_rows:,}", flush=True)
print(f"  enhancer links: {n_enhancer_rows:,}", flush=True)
print(f"  eRegulons: {n_eregulons:,}", flush=True)

# --- Biological sanity: PBMC + granulocyte-relevant TFs ---
# Myeloid lineage (granulocytes, monocytes): SPI1 (PU.1), CEBPA, CEBPB, CEBPE
# Lymphoid: PAX5 (B), GATA3 (CD4 T), TBX21 (NK/CD8 T), EBF1 (B)
# Erythroid/megakaryocyte (in unsorted granulocyte preps): KLF1, GATA1
expected_tfs = [
    "SPI1", "CEBPA", "CEBPB", "CEBPE",
    "PAX5", "GATA3", "TBX21", "EBF1",
    "IRF8", "FOXP3",
]
import json as _json
regulons_json = _json.loads(Path(result.regulons_path).read_text())

def _strip_suffixes(name):
    s = name
    while True:
        prev = s
        for suf in ("_regulon", "_extended", "_activator", "_repressor"):
            if s.endswith(suf): s = s[:-len(suf)].strip()
        for paren in ("(+)", "(-)"):
            if s.endswith(paren): s = s[:-3].strip()
        if s == prev: return s

regulon_tfs = {_strip_suffixes(k) for k in regulons_json.keys()}
found_tfs = sorted([t for t in expected_tfs if t in regulon_tfs])
missing_tfs = sorted([t for t in expected_tfs if t not in regulon_tfs])
print(f"\n=== biological sanity (PBMC + granulocyte canonical TFs) ===", flush=True)
print(f"  expected: {expected_tfs}", flush=True)
print(f"  found in regulons: {found_tfs}", flush=True)
print(f"  missing: {missing_tfs}", flush=True)

# --- Cross-check: did the v0.4.3 PipelineResult fields make sense? ---
v043_field_checks = {
    "pruned_regulons_path_is_none_when_no_pruning": (
        n_pruned_regulons is None and getattr(result, "pruned_regulons_path", None) is None
    ),
    "candidate_regulons_path_set": getattr(result, "candidate_regulons_path", None) is not None,
    "regulon_source_is_a_string": isinstance(regulon_source, str) and len(regulon_source) > 0,
    "n_regulons_equals_n_candidates_when_no_pruning": (
        n_pruned_regulons is None and n_regulons == n_candidate_regulons
    ),
}
print(f"\n=== v0.4.3 field-contract checks ===", flush=True)
for k, v in v043_field_checks.items():
    print(f"  {k}: {v}", flush=True)

# --- Hard assertions ---
all_nonempty = (
    n_grn_edges > 0 and
    n_regulons > 0 and
    n_cistarget_rows > 0 and
    n_enhancer_rows > 0 and
    n_eregulons > 0 and
    result.integrated_adata_path is not None and
    Path(result.integrated_adata_path).exists()
)
assert all_nonempty, (
    f"FAIL: at least one stage produced empty output. "
    f"GRN={n_grn_edges}, regulons={n_regulons}, cistarget={n_cistarget_rows}, "
    f"enhancer={n_enhancer_rows}, eRegulons={n_eregulons}, "
    f"integrated={result.integrated_adata_path}"
)

assert all(v043_field_checks.values()), (
    f"FAIL: v0.4.3 field-contract check failed: {v043_field_checks}"
)

artefact = {
    "release": os.environ["RUSTSCENIC_VER"],
    "smoke_type": (
        "real-data full SCENIC+ E2E via public pipeline.run — "
        "third independent dataset (PBMC granulocyte-sorted 10k human multiome)"
    ),
    "rustscenic_version": os.environ["RUSTSCENIC_VER"],
    "rustscenic_sha": os.environ["RUSTSCENIC_SHA"],
    "rustscenic_branch": os.environ["RUSTSCENIC_BRANCH"],
    "install_command": f"pip install \"{os.environ['HOME']}/projects/bio/rustscenic[validation]\"",
    "api_call": (
        "rustscenic.pipeline.run(rna=adata, output_dir=..., adata_atac=adata_atac, "
        "motif_rankings=hs_rankings, gene_coords=hs_coords, tfs='hs', "
        "grn_n_estimators=100, topics_n_topics=15, topics_n_passes=3, "
        "eregulon_min_target_genes=2, eregulon_min_enhancer_links=1, seed=777)"
    ),
    "dataset": {
        "name": "10x pbmc_granulocyte_sorted_10k",
        "source": "cf.10xgenomics.com/samples/cell-arc/2.0.0/pbmc_granulocyte_sorted_10k",
        "species": "Homo sapiens (hg38)",
        "tissue": "peripheral blood mononuclear cells + granulocytes (sorted)",
        "rna_h5_md5": md5(RNA_H5),
        "atac_fragments_md5_first_8mb": hashlib.md5(open(ATAC_FRAG,"rb").read(8*1024*1024)).hexdigest(),
        "peaks_bed_md5": md5(PEAKS_BED),
    },
    "shapes": {
        "rna_post_qc": list(rna.shape),
        "atac_subset_to_rna_cells": list(adata_atac.shape),
    },
    "wall_s": {
        "setup": round(setup_wall, 2),
        "pipeline_run_total": round(pipeline_wall, 2),
    },
    "peak_rss_gb": round(peak_rss_gb, 2),
    "outputs_non_empty": {
        "grn": n_grn_edges > 0,
        "regulons": n_regulons > 0,
        "cistarget": n_cistarget_rows > 0,
        "enhancer_links": n_enhancer_rows > 0,
        "eregulons": n_eregulons > 0,
        "integrated_adata": result.integrated_adata_path is not None and Path(result.integrated_adata_path).exists(),
    },
    "headline_counts": {
        "n_grn_edges": n_grn_edges,
        "n_candidate_regulons": n_candidate_regulons,
        "n_regulons": n_regulons,
        "n_pruned_regulons": n_pruned_regulons,
        "n_cistarget_rows": n_cistarget_rows,
        "n_enhancer_links": n_enhancer_rows,
        "n_eregulons": n_eregulons,
    },
    "regulon_source": regulon_source,
    "biological_sanity": {
        "expected_pbmc_granulocyte_tfs": expected_tfs,
        "found_in_regulons": found_tfs,
        "missing_from_regulons": missing_tfs,
        "fraction_recovered": round(len(found_tfs) / len(expected_tfs), 3),
    },
    "v043_field_contract_checks": v043_field_checks,
    "output_inventory": output_inventory,
    "elapsed_per_stage": result.elapsed if hasattr(result, "elapsed") else {},
    "env": {
        "python": os.environ["PYVER"],
        "scanpy": os.environ["SCANPY_VER"],
        "anndata": os.environ["ANNDATA_VER"],
        "os": os.environ["OS"],
        "cpu": os.environ["CPU"],
        "n_cpus": int(os.environ["N_CPUS"]),
    },
    "supported_claims": [
        "rustscenic v0.4.3 pipeline.run completes E2E on a third public real-data multiome (PBMC granulocyte-sorted 10k human)",
        "All 7 SCENIC+ stages produce non-empty output on this dataset (GRN, regulons, AUCell, topics, cistarget, enhancer-links, eRegulons + integrated AnnData)",
        "PipelineResult v0.4.3 field-contract holds on the no-pruning path: pruned_regulons_path is None, n_regulons == n_candidate_regulons, regulon_source is a non-empty string",
        f"Of {len(expected_tfs)} canonical PBMC+granulocyte TFs (myeloid + lymphoid), {len(found_tfs)} are present in the active regulon set by name (not cell-type enrichment)",
    ],
    "scope_notes": [
        "Third real multiome dataset on record after PBMC 3k (v0.3.9) and mouse brain E18 5k (v0.3.10).",
        "Different cell composition than the prior PBMC 3k run: granulocyte-sorted, so the myeloid TF axis (SPI1/CEBPA/CEBPB) gets tested in addition to lymphoid TFs (PAX5/GATA3/TBX21).",
        "Pre-pipeline setup time (RNA QC, ATAC subset, motif/gene-coords download) recorded separately from pipeline.run wall.",
        "ATAC matrix pre-subset to RNA-QC'd cells (the v0.3.8 adata_atac path).",
        "GRN n_estimators=100 is the smoke speed; README's quoted parity number uses n_estimators=5000.",
        "eRegulon thresholds permissive (min_target_genes=2, min_enhancer_links=1) for orchestration proof.",
        "Topics method=vb (default), n_topics=15 to reflect PBMC+granulocyte diversity.",
        "Biological sanity is name-presence in the regulon set, NOT cell-type enrichment. The latter requires a cluster-vs-AUCell test which is tracked as a follow-up.",
        "Motif-annotation pruning (v0.4.2 feature) NOT exercised here — motif_annotations argument not supplied, so pruned_regulons_path should be None and regulon_source should be 'candidate_grn_top_targets'. v0.4.3 fix verified: the path is None, not a stale empty-dict file.",
    ],
    "open_gates": [
        "Cell-type enrichment of recovered TFs: requires AnnData with cluster labels + AUCell-per-cluster F-test or ARI-vs-RNA-cluster. Tracked for follow-up.",
        "Cistarget motif-annotation pruning + NES path on this dataset: deferred to v0.4.4 (NES PR).",
        "Cross-tool parity vs pycistarget on this dataset: blocked on pycistarget's installability (the very gap rustscenic exists to fix).",
    ],
}

art_path = Path(os.environ["ARTEFACT"])
art_path.write_text(json.dumps(artefact, indent=2))
print(f"\nartefact → {art_path}", flush=True)
print(json.dumps(artefact, indent=2), flush=True)
PY

deactivate
echo "done"
