...
Subsamples the same real dataset (Gland Atlas, 50,240 cells) at
multiple cell counts. Same TFs, same n_estimators. Reports per-call
wall-clock and the ratio vs the smallest size.
"""
...
def main() -> int:
    h5ad_path = HERE / "gland_atlas_50k.h5ad"
    ...
    for n in cell_counts:
        ...
        with warnings.catch_warnings():
            ...
            grn = rustscenic.grn.infer(
                sub, tf_names=candidate_tfs, n_estimators=20, seed=0, verbose=False,
            )
            dt = time.monotonic() - t0
        # Estimate "compute units": cells × genes × tfs × n_estimators (constant TFs/n_est here).
        compute_units = n * sub.n_vars * len(set(grn["TF"].unique())) * 20
        results.append({
            "n_cells": n,
            "n_genes": sub.n_vars,
    ...
    out = HERE / "grn_scaling_curve.json"
