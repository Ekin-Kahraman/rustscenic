...
def test_real_multiome_scaling_child_row_carries_child_validation_fields(
    ...
    child_record = {
        ...
        "backend_execution": {
            "pipeline_grn": {"engine": "rust", "required_ok": True, "symbol_count": 1}
        },
...
def test_minerva_preflight_requires_default_reference_cache(monkeypatch, tmp_path):
    module = _load_module(
        "preflight_minerva_reference_cache",
        ROOT / "validation/hpc/minerva/preflight_minerva.py",
    )
    repo, env, data = _write_minerva_preflight_fixture(tmp_path)
    monkeypatch.setattr(
    ...
    assert result["ok"] is False
    assert (
        f"missing default reference cache motif_rankings: "
        f"{tmp_path / 'missing_motifs.feather'}"
    ) in result["failures"]
...
def test_minerva_preflight_reports_default_reference_cache_paths(monkeypatch, tmp_path):
    ...
    result = module._reference_cache_statuses(
        args,
    ...
    assert result["ok"] is True
    assert result["motif_rankings"]["needed"] is True
    assert result["motif_rankings"]["source"] == "default_cache"
    assert result["motif_rankings"]["path"].endswith(
...
def test_benchmark_artifact_validator_rejects_full_file_when_projection_possible(tmp_path):
    module = _load_module(
        "validate_benchmark_artifact_requires_projected_cistarget",
        ROOT / "validation/hpc/minerva/validate_benchmark_artifact.py",
    )
    record = _full_pipeline_record(tmp_path)
    record["cistarget_rankings"] = {
    ...
    record["backend_execution"].pop("pipeline_cistarget_ranking_projection", None)
    _sync_full_pipeline_manifest(record)

    failures = module.validate_record(record, require_clean=True)

    assert (
        "full_pipeline.cistarget_rankings.mode must be projected_file when "
        "full_pipeline.reference_fingerprints.motif_rankings.file_backed is true "
        "and full_pipeline.cistarget_rankings.requested_features is smaller than "
        "full_pipeline.cistarget_rankings.rank_universe_size"
    ) in failures
...
def test_benchmark_artifact_validator_rejects_full_region_file_when_projection_possible(tmp_path):
    module = _load_module(
        "validate_benchmark_artifact_requires_projected_region_rankings",
        ROOT / "validation/hpc/minerva/validate_benchmark_artifact.py",
    )
    record = _full_pipeline_record(tmp_path)
    record["params"]["region_motif_rankings"] = "region_rankings.feather"
    ...
    record["shapes"]["region_motif_rankings"] = [8, 2000]
    record["region_cistarget_rankings"] = {
        "input_kind": "feather",
        "mode": "full_file",
        "projected": False,
        "loaded_columns": 2000,
    ...
    assert (
        "full_pipeline.region_cistarget_rankings.mode must be projected_file when "
        "full_pipeline.reference_fingerprints.region_motif_rankings.file_backed is true "
        "and full_pipeline.region_cistarget_rankings.requested_features is smaller than "
        "full_pipeline.region_cistarget_rankings.rank_universe_size"
    ) in failures
...
def test_minerva_launchers_validate_benchmark_artifacts_after_run():
    ...
    assert "--require-reference-cache" in readme
...
def test_audit_workflow_runs_backend_and_hot_path_preflights():
    ...
    assert workflow.index("python -m pip install target/wheels/rustscenic-*.whl") < workflow.index(
        "python -m rustscenic doctor --json"
    )
    assert workflow.index("python -m rustscenic doctor --json") < workflow.index(
        "python validation/python_hot_paths.py python/rustscenic --json"
    )
    assert workflow.index("python validation/python_hot_paths.py python/rustscenic --json") < workflow.index(
        "pytest tests/ -v"
    )
